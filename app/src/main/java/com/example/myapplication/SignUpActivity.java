...
public class SignUpActivity extends AppCompatActivity {
    ...
    private EditText  et_email, et_pw, et_pw2, et_signName, et_nickname;      //회원가입 입력필드
    private Button pwcheck, submit;     //비밀번호 같은지확인, 회원가입버튼
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        et_signName = findViewById(R.id.et_signName);
        ...
        pwcheck.setOnClickListener(view -> {
            password = et_pw.getText().toString(); // 패스워드
            pwck = et_pw2.getText().toString(); // 패스워드 체크 여부
            if(isValidPasswd()) {
                pwcheck.setText("일치");
    ...
    public void singUp(View view) {
        email = et_email.getText().toString()+"@office.deu.ac.kr"; // 이메일
        signName = et_signName.getText().toString(); //이름
        password = et_pw.getText().toString(); // 패스워드
        pwck = et_pw2.getText().toString(); // 패스워드 체크 여부
        nick = et_nickname.getText().toString(); // 닉네임 체크 여부
        // 모두 체크 완료 됐을때 회원가입 진행
        if(isValidEmail() && isVaildName_nick() && isValidPasswd() ) {
    ...
    private boolean isValidPasswd() {
        if (password.isEmpty()) {
            ...
            Toast.makeText(SignUpActivity.this, "비밀번호 형식이 불일치 합니다.", Toast.LENGTH_SHORT).show();
            // 비밀번호 형식 불일치
            return false;
    ...
    // 이름, 닉네임 넣었는지 검사
    private boolean isVaildName_nick(){
        if (signName.isEmpty()){
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            User.setEmailI(email); // 유저 이메일 저장
                            User.setPassword(password); // 유저 pw저장
                            User.setCheck_count(0); //카운트 저장
                            User.setNickname(nick); //닉네임 저장
